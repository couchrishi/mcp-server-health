...
from utils.gemini_analysis_utils import analyze_file_list, analyze_server_readme
...
async def fetch_github_basic_stats(client: httpx.AsyncClient, repo_url: str, semaphore: asyncio.Semaphore) -> Dict[str, Any]:
    """Fetch basic stats (stars, forks, watchers) for a GitHub repository."""
    # Extract owner and repo name from the URL
    # Example URL: https://github.com/MindscapeHQ/mcp-server-raygun
...
async def fetch_repo_files(client: httpx.AsyncClient, repo_url: str, semaphore: asyncio.Semaphore) -> Dict[str, Any]:
...
async def enrich_server_with_gemini(gemini_model, file_list: List[str], readme_content: str) -> Dict[str, Any]:
...
async def enrich_server_data():
    """Enrich the discovered server data with additional metadata."""
    logger.info("--- Starting Server Data Enrichment with Gemini ---")
    ...
    async with httpx.AsyncClient(timeout=config.REQUEST_TIMEOUT, headers=headers) as client:
        for category, servers in data["servers"].items():
            ...
            for server in servers:
                # Fetch basic stats from GitHub API
                basic_stats = await fetch_github_basic_stats(client, server["repo_url"], semaphore)
                
                # Skip non-GitHub URLs or URLs with errors
                if basic_stats.get("error") and "Could not parse GitHub URL" in basic_stats["error"]:
                ...
                # Fetch file list and README content
                repo_files = await fetch_repo_files(client, server["repo_url"], semaphore)
                ...
                analysis_results = {
                    ...
                    "packages": gemini_analysis["packages"]
                }
...
if __name__ == "__main__":
    asyncio.run(enrich_server_data())